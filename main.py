...
def validate_user():
    user = str(self.request.cookies.get('name'))
    q = Post.id.author()
    author = q.get()
...
class NewPost(BlogHandler):
    ...
    def post(self):
        ...
        author = str(self.request.cookies.get('name'))
...
class User(db.Model):
    ...
    @classmethod
    def by_id(cls, uid, parent=users_key()):
    ...
    @classmethod
    def register(cls, name, pw, email=None, parent=users_key()):
...
class EditPost(BlogHandler):
    ...
    def post(self, post_id):
        ...
        subject = self.request.get("subject")
        content = self.request.get("content")
        param_list = dict(post=post, subject=subject, content=content)

        have_error = False

        if not subject:
            param_list['subject_error'] = "Subject is missing"
            have_error = True
        if not content:
        ...
        if have_error:
            ...
            p = Post.update_post(int(post_id), subject, content)
